...
Race manager: Do this first as might be used by the other classes.
...
*Need to think about set up periodisation phases so that can set phase I, II, III, IV etc. (May need to be able to change this for other periodisation methods/ philosophies)... Maybe set by race date, so can set goal race, then put starting period and then calculate phases from that.
...
***PHASES***
Enter date or mark race date as the target date, enter training start date. Option to include phase I or skip...
...
*Find a graphing gem for displaying stats. - Chartkick seems nice and easy to use.x

*Redesign the races functionality so radio boxes are used for the race distances when entering.

*Work on races part so that default distances can be selected, but also new ones can be added. - partially, radio buttons implemented, will need to consider whether to add other and text entry functionality.
